...
#   -Z, --zx0        Enable ZX0 super-compression (slower build, smaller binary)
#   --fbdev          Use Framebuffer backend (recommended & universal: boot + shutdown, static)
#   --drm            Use DRM/KMS backend (specialized for boot, tear-free VSync)
#
...
#   --check-silent-boot           Check silent boot configuration in GRUB
#   --setup-silent-boot           Check and configure silent boot in GRUB
#
# Input:
...
BG_OFFSET_Y=0
TARGET_RES=""
LOOP_MODE=1  # 0=no loop, 1=full loop, 2=partial loop, 3=ping-pong
LOOP_START=0  # Start frame for partial loop
INVERT_FRAMES=0  # 0=normal order (0→N), 1=inverted order (N→0)
USE_ZX0=0  # 0=normal (fast, LZSS/direct), 1=ZX0 super-compression
...
# Normalize relative path arguments against original working directory before cd to REPO_ROOT
if [ -n "${FRAME_DIR:-}" ] && [[ ! "$FRAME_DIR" = /* ]]; then
    FRAME_DIR="$ORIG_CWD/$FRAME_DIR"
fi
if [ -n "${BG_IMAGE:-}" ] && [[ ! "$BG_IMAGE" = /* ]]; then
    BG_IMAGE="$ORIG_CWD/$BG_IMAGE"
fi
if [ -n "${INSTALL_PACKAGE:-}" ] && [[ ! "$INSTALL_PACKAGE" = /* ]]; then
    INSTALL_PACKAGE="$ORIG_CWD/$INSTALL_PACKAGE"
fi
if [ -n "${INSTALL_EXISTING:-}" ] && [[ ! "$INSTALL_EXISTING" = /* ]]; then
    INSTALL_EXISTING="$ORIG_CWD/$INSTALL_EXISTING"
fi
cd "$REPO_ROOT"
...
validate_binary() {
    ...
    if [[ ${USE_DRM:-0} -eq 0 ]]; then
        if LC_ALL=C ldd "$binary" 2>&1 | grep -v "not a dynamic executable" >/dev/null 2>&1; then
...
check_kernel_cmdline() {
    ...
    # Check each recommended option (splash is optional legacy flag)
    for opt in quiet loglevel=0 vt.cur_default=1; do
...
analyze_frames() {
    ...
    # Find image files (PNG, JPG, JPEG) using natural version sort
    local all_files=()
    while IFS= read -r f; do
        [ -n "$f" ] && all_files+=("$f")
    done < <(find "$dir" -maxdepth 1 -type f \( -name "*.png" -o -name "*.PNG" -o -name "*.jpg" -o -name "*.JPG" -o -name "*.jpeg" -o -name "*.JPEG" \) 2>/dev/null | sort -V)
    ...
    # Separate animation frames and non-animation/intruder images
    local frame_files=()
    local intruder_files=()
    
    # Check if files have digits
    local files_with_digits=0
    for file in "${all_files[@]}"; do
        local bn=$(basename "$file")
        if [[ "$bn" =~ [0-9] ]]; then
            files_with_digits=$((files_with_digits + 1))
        fi
    done
    
    for file in "${all_files[@]}"; do
        local bn=$(basename "$file")
        local lower_bn=$(echo "$bn" | tr '[:upper:]' '[:lower:]')
        
        # Check keyword matches or missing digit when majority have digits
        if [[ "$lower_bn" =~ (background|bg_|bg-|static|preview|thumb|thumbnail|palette|screenshot|cover|poster) ]]; then
            intruder_files+=("$bn")
        elif [ "$files_with_digits" -gt 0 ] && ! [[ "$bn" =~ [0-9] ]]; then
            intruder_files+=("$bn")
        else
            frame_files+=("$file")
        fi
    ...
    # If all files were marked as intruders (e.g. naming without digits), restore them
    if [ ${#frame_files[@]} -eq 0 ] && [ ${#intruder_files[@]} -gt 0 ]; then
        frame_files=("${all_files[@]}")
        intruder_files=()
    fi
    
    # Display excluded images if any
    if [ ${#intruder_files[@]} -gt 0 ]; then
        echo -e "  ${YELLOW}╭──────────────────────────────────────────────────────────────────────────────╮${NC}"
        echo -e "  ${YELLOW}│${CYAN}  Excluded Non-Animation Image(s):                                             ${YELLOW}│${NC}"
        for f in "${intruder_files[@]}"; do
            printf "  ${YELLOW}│${NC}    • %-72s ${YELLOW}│${NC}\n" "$f"
        done
        echo -e "  ${YELLOW}╰──────────────────────────────────────────────────────────────────────────────╯${NC}"
    fi
    
    local img_count=${#frame_files[@]}
    print_success "Found $img_count valid animation frames"
    
    # Frame dimensions from first valid frame
    local first_frame=$(basename "${frame_files[0]}")
    local last_frame=$(basename "${frame_files[$((img_count - 1))]}")
    local frame_size=""
    if identify "${frame_files[0]}" &>/dev/null; then
        frame_size=$(identify -format "%wx%h" "${frame_files[0]}" 2>/dev/null) || true
    fi
    ...
    echo -e "     ${CYAN}First frame:${NC}     ${BOLD}$first_frame${NC}"
    echo -e "     ${CYAN}Last frame:${NC}      ${BOLD}$last_frame${NC}"
    echo -e "     ${GREEN}════════════════════════════════════════════════════════════════${NC}"
...
select_mode() {
    ...
    echo -e "  ${YELLOW_GRAD3}│      ${YELLOW}→ Animation frames overlaid on a static image on uniform color${NC}          ${YELLOW_GRAD3}│"
    ...
    echo -e "  ${YELLOW_GRAD3}│      ${YELLOW}→ Single static image on uniform color${NC}                                  ${YELLOW_GRAD3}│"
    ...
    local mode_names=("Animation on solid background" "Animation on background image on uniform color" 
...
get_parameters() {
    ...
    if [ $DISPLAY_MODE -le 2 ]; then
        ...
        while true; do
            ...
            echo -e "  ${YELLOW_GRAD2}│  ${YELLOW}4)${NC} ${BOLD}Ping-Pong${NC}     - Play forward then backward smoothly       ${YELLOW_GRAD2}│"
    ...
    if [ $DISPLAY_MODE -le 2 ]; then
        ...
        case $LOOP_MODE in
            ...
            3) loop_desc="Ping-pong loop (${dir_arrow}, then reverse...)" ;;
        esac
    ...
    if [ $DISPLAY_MODE -le 2 ]; then
        ...
        case $LOOP_MODE in
            ...
            3) echo -e "   ${CYAN}Loop mode:${NC}         Ping-pong loop (forward ↔ backward)" ;;
        esac
    elif [ $DISPLAY_MODE -eq 3 ]; then
...
build_animation() {
    ...
    if [ ! -f "generate_splash" ] || [ "engine_src/generate_splash.c" -nt "generate_splash" ]; then
        print_info "⚙ Compiling splash generator..."
        rm -f generate_splash
        make generator || gcc -O2 -Iengine_src -I. -o generate_splash engine_src/generate_splash.c -lpng -lm
    ...
    grep -E "^(Display|Found|Frame|Offsets|Background|Image|Total|Resizing|ZX0|Super-packing|Compressing)" build.log | while read -r line; do
        print_info "  $line"
    done
    ...
    if [[ $USE_DRM -eq 1 ]]; then
        echo -e "  ${CYAN}Backend:${NC}         ${GREEN}DRM/KMS${NC} (Specialized for Boot, tear-free VSync, dynamic libdrm)"
    else
        echo -e "  ${CYAN}Backend:${NC}         ${GREEN}Framebuffer fbdev${NC} (Recommended & Universal: Boot + Shutdown, static, 0 deps)"
    fi
...
_update_frame_cache() {
    ...
    local _cs=""
    local _ro_hdr=$(readelf -W -S "$_bin" 2>/dev/null | grep '\.rodata\.cfg')
    if [ -n "$_ro_hdr" ]; then
        local _wm_off=$(echo "$_ro_hdr" | awk '{for(i=1;i<=NF;i++) if($i==".rodata.cfg") print $(i+3)}')
        if [ -n "$_wm_off" ]; then
            _cs=$(dd if="$_bin" bs=1 skip=$(( 16#$_wm_off )) count=24 2>/dev/null | od -An -tx1 | tr -d ' \n')
        fi
    fi
    if [ -z "$_cs" ] || [ ${#_cs} -lt 40 ]; then
        local _scan_off=$(grep -b -a -o $'\x19\xb2\xf3\xa7' "$_bin" 2>/dev/null | head -1 | cut -d: -f1)
        if [ -n "$_scan_off" ]; then
            _cs=$(dd if="$_bin" bs=1 skip="$_scan_off" count=24 2>/dev/null | od -An -tx1 | tr -d ' \n')
        fi
    fi
    ...
    if [ -n "$FRAME_CRC" ]; then
        ...
        [ $(( _wc ^ FRAME_CRC )) -ne 0 ] && _e=$((_e+1))
    fi
    
    # Verify .rodata size is plausible for frame data
    local _ro_sz=$(readelf -W -S "$_bin" 2>/dev/null | grep -E '[[:space:]]\.rodata[[:space:]]' | awk '{for(i=1;i<=NF;i++) if($i==".rodata") print $(i+4)}' | head -1)
    if [ -n "$_ro_sz" ]; then
        _ro_sz=$(( 16#$_ro_sz ))
...
install_standard() {
    ...
    local installed_files=(
        ...
        "/etc/initramfs-tools/scripts/local-premount/00_$BINARY"
    )
    local backup_files=(
        "/sbin/${BINARY}.bak"
        ...
        "/etc/initramfs-tools/scripts/local-premount/00_${BINARY}.bak"
    )
    local grub_backup=""
...
INIT_BOTTOM_FBDEV
    ...
    # Create local-premount script for hibernation resume notification
    mkdir -p /etc/initramfs-tools/scripts/local-premount
    if [[ -f /etc/initramfs-tools/scripts/local-premount/00_$BINARY ]]; then
        cp /etc/initramfs-tools/scripts/local-premount/00_$BINARY /etc/initramfs-tools/scripts/local-premount/00_${BINARY}.bak
    fi
    local resumepre_tmp="/etc/initramfs-tools/scripts/local-premount/00_${BINARY}.tmp"
    cat > "$resumepre_tmp" << 'RESUME_PRE_EOF'
#!/bin/sh
# Early notification for __BINARY__ when resuming from hibernation (swsusp)
PREREQ=""
prereqs() { echo "$PREREQ"; }
case "$1" in prereqs) prereqs; exit 0;; esac
...
if [ "$(get_fstype "${DEV}")" = "suspend" ]; then
    if [ -f /run/__BINARY__.pid ]; then
        PID=$(cat /run/__BINARY__.pid 2>/dev/null)
        if [ -n "$PID" ] && [ -d "/proc/$PID" ]; then
            COMM=$(cat /proc/$PID/comm 2>/dev/null)
            if [ "$COMM" = "__BINARY__" ]; then
...
RESUME_PRE_EOF
    sed -i "s/__BINARY__/$BINARY/g" "$resumepre_tmp"
    chmod 755 "$resumepre_tmp"
    mv "$resumepre_tmp" /etc/initramfs-tools/scripts/local-premount/00_$BINARY
    echo "  -> /etc/initramfs-tools/scripts/local-premount/00_$BINARY"
...
install_shutdown() {
    ...
    # Install binary
    echo -e "${GREEN}[2/3]⚙ Installing binary to /sbin...${NC}"
    mkdir -p /sbin
    
    local sbin_binary="/sbin/$BINARY"
    if [[ -f "$sbin_binary" ]]; then
        cp "$sbin_binary" "${sbin_binary}.bak"
        echo "  -> Backed up existing binary"
    fi
    
    cp "$BINARY" "$sbin_binary"
    chmod +x "$sbin_binary"
    echo "  -> $sbin_binary"
...
do_uninstall() {
    ...
    if [ ${#installed_binaries[@]} -eq 0 ] && [ ${#shutdown_binaries[@]} -eq 0 ]; then
        echo -e "${YELLOW}No xbs_* or xbootsplash* binaries found in /sbin or /lib/systemd/system-shutdown${NC}"
    else
        ...
        case "$remove_choice" in
            ...
            [sS])
                ...
                if [[ "$selection" =~ ^[0-9]+$ ]] && [[ "$selection" -ge 1 ]] && [[ "$selection" -le ${#all_bins[@]} ]]; then
                    ...
                    if [[ " ${installed_binaries[*]} " =~ " ${selected_bin} " ]]; then
                        ...
                        rm -f /etc/initramfs-tools/scripts/local-premount/00_$selected_bin 2>/dev/null || true
                        rm -f /etc/initramfs-tools/scripts/local-premount/00_${selected_bin}.bak 2>/dev/null || true
            ...
            *)
                ...
                for bin in "${installed_binaries[@]}"; do
                    ...
                    rm -f /etc/initramfs-tools/scripts/local-premount/00_$bin 2>/dev/null && echo "  -> Removed local-premount" || true
                    rm -f /etc/initramfs-tools/scripts/local-premount/00_${bin}.bak 2>/dev/null || true
...
generate_preview() {
    ...
    if ask_yes_no "Generate animated GIF preview (800px width)?"; then
        ...
        # Build frame list sorted naturally to ensure correct order
        local raw_list=($(find "$FRAME_DIR" -maxdepth 1 \( -name "*.png" -o -name "*.PNG" \) -type f 2>/dev/null | sort -V))
        local frames_list=()
        for f in "${raw_list[@]}"; do
            local bn=$(basename "$f" | tr '[:upper:]' '[:lower:]')
            if [[ "$bn" =~ (background|bg_|bg-|static|preview|thumb|thumbnail|palette|screenshot|cover|poster) ]]; then
                continue
            fi
            frames_list+=("$f")
        done
        [ ${#frames_list[@]} -eq 0 ] && frames_list=("${raw_list[@]}")
        ...
        # For ping-pong loop mode, append frames in reverse (N-2 down to 1) for GIF preview
        if [ "$LOOP_MODE" -eq 3 ] && [ "$frame_count" -gt 2 ]; then
            print_info "Ping-pong loop: appending reverse frames for preview..."
            local append_idx=$frame_count
            for ((rev_i = frame_count - 2; rev_i >= 1; rev_i--)); do
                local src_f="${tmp_dir}/frame_$(printf '%04d' $rev_i).png"
                if [ -f "$src_f" ]; then
                    cp "$src_f" "${tmp_dir}/frame_$(printf '%04d' $append_idx).png"
                    append_idx=$((append_idx + 1))
                fi
            done
            local total_frames=$append_idx
            print_info "Preview will have $total_frames frames (forward + backward)"
        fi
        ...
        if "${cmd[@]}" 2>build.log; then
            ...
            case $LOOP_MODE in
                ...
                3) print_info "Loop: Ping-Pong (forward ↔ backward)" ;;
            esac
        else
            rm -rf "$tmp_dir" "$tmp_bg"
...
export_xbs_package() {
    ...
    if [ -f "$preview_file" ]; then
        if [ "${DISPLAY_MODE:-0}" -ge 2 ]; then
            # Static image mode: prefer clean PNG
            if [[ "$preview_file" == *.png ]]; then
                cp "$preview_file" "$tmp_dir/preview.png" 2>/dev/null
            else
                cp "$preview_file" "$tmp_dir/preview.gif" 2>/dev/null
            fi
        else
            # Animation mode: prefer animated GIF
    ...
    local binary_name=$(basename "$BINARY")
    local binary_size=$(stat -c%s "$BINARY" 2>/dev/null || wc -c < "$BINARY" | tr -d ' ')
    ...
    # Determine compression technique description
    local comp_desc="RLE (Auto)"
    if [ "$DISPLAY_MODE" -ge 3 ]; then
        if [ "${USE_ZX0:-0}" -eq 1 ]; then
            comp_desc="ZX0 (Palette 8-bit)"
        else
            comp_desc="LZSS (Palette 8-bit)"
        fi
    else
        local base_comp="RLE"
        if [ -n "$COMPRESS_METHOD" ]; then
            case "$COMPRESS_METHOD" in
                0|rle_xor) base_comp="RLE XOR" ;;
                1|rle_direct) base_comp="RLE Direct" ;;
                2|sparse) base_comp="Sparse XOR" ;;
                3|raw) base_comp="Raw XOR" ;;
                auto) base_comp="RLE (Auto)" ;;
                *) base_comp="$COMPRESS_METHOD" ;;
            esac
        fi
        if [ "${USE_ZX0:-0}" -eq 1 ]; then
            comp_desc="ZX0 Super-pack ($base_comp)"
        else
            comp_desc="$base_comp (Delta)"
        fi
    fi
...
install_from_package() {
    ...
    case "$DISPLAY_MODE" in
        ...
        1) mode_desc="Animation on background image on uniform color" ;;
    ...
    # Detect compression if not in metadata
    if [ -z "$COMPRESSION" ] && [ -f "$tmp_dir/splash_bin" ]; then
        local _off=$(grep -b -a -o $'\x19\xb2\xf3\xa7' "$tmp_dir/splash_bin" 2>/dev/null | head -1 | cut -d: -f1)
        if [ -n "$_off" ]; then
            local _c=$(dd if="$tmp_dir/splash_bin" bs=1 skip=$((_off + 14)) count=2 2>/dev/null | od -An -tu2 | tr -d ' ')
            case "$_c" in
                0) COMPRESSION="RLE XOR (Delta)" ;;
                1) COMPRESSION="RLE Direct (Delta)" ;;
                2) COMPRESSION="Sparse XOR (Delta)" ;;
                3) COMPRESSION="Raw (Uncompressed)" ;;
                5) COMPRESSION="LZSS (Palette 8-bit)" ;;
                6) COMPRESSION="ZX0 (Palette 8-bit)" ;;
            esac
        fi
    fi
    ...
    if [ -n "$COMPRESSION" ]; then
        echo -e "  ${YELLOW}Compression:${NC} $COMPRESSION"
    fi
    ...
    local bin_size_display=""
    if [ -f "$tmp_dir/splash_bin" ]; then
        local bsz=$(stat -c%s "$tmp_dir/splash_bin" 2>/dev/null || wc -c < "$tmp_dir/splash_bin" | tr -d ' ')
        local bsz_kb=$(( (bsz + 1023) / 1024 ))
        bin_size_display="${bsz_kb} KB (${bsz} bytes)"
    fi
    if [ -n "$bin_size_display" ]; then
        echo -e "  ${YELLOW}Binary size:${NC} $bin_size_display"
    fi
    local pkg_sz_display=$(du -h "$pkg_file" 2>/dev/null | cut -f1)
    if [ -n "$pkg_sz_display" ]; then
        echo -e "  ${YELLOW}Package size:${NC}$pkg_sz_display"
    fi
...
extract_binary() {
    ...
    local extract_tool=""
    if [ -x "$REPO_ROOT/extract_frames" ]; then
        extract_tool="$REPO_ROOT/extract_frames"
    elif [ -x "$SCRIPT_DIR/extract_frames" ]; then
        extract_tool="$SCRIPT_DIR/extract_frames"
    elif [ -f "$REPO_ROOT/extract_frames.c" ]; then
        print_info "Building extract_frames tool..."
        gcc -O2 -o "$REPO_ROOT/extract_frames" "$REPO_ROOT/extract_frames.c" -lpng 2>/dev/null
        [ -x "$REPO_ROOT/extract_frames" ] && extract_tool="$REPO_ROOT/extract_frames"
    elif [ -f "$SCRIPT_DIR/extract_frames.c" ]; then
        print_info "Building extract_frames tool..."
        gcc -O2 -o "$SCRIPT_DIR/extract_frames" "$SCRIPT_DIR/extract_frames.c" -lpng 2>/dev/null
        [ -x "$SCRIPT_DIR/extract_frames" ] && extract_tool="$SCRIPT_DIR/extract_frames"
    fi
    ...
    if [ -x "$extract_tool" ]; then
        ...
        if [ -n "$wm_info" ]; then
            ...
            if [ -n "$wm_hex" ] && [ ${#wm_hex} -ge 48 ]; then
                ...
                # Mode names
                local mode_names=("Anim solid bg" "Anim on static bg" "Static on solid bg" "Static fullscreen" "Anim image fullscreen")
...
main() {
    ...
    # Check graphics backends and set default mode
    print_info "  ${WHITE}☉${CYAN} Checking graphics backends..."
    check_libdrm
    if [[ $USE_DRM_EXPLICIT -eq 0 ]]; then
        USE_DRM=0  # Framebuffer fbdev is the recommended universal default (Boot + Shutdown)
    fi
    echo -e "    ${GREEN}✓ Framebuffer (/dev/fb0):${NC} ${WHITE}Recommended & Universal (Boot + Shutdown, static, 0 deps)${NC}"
    if [[ $LIBDRM_AVAILABLE -eq 1 ]]; then
        echo -e "    ${CYAN}ℹ DRM/KMS (/dev/dri):${NC}      ${WHITE}Specialized for Boot (tear-free VSync, dynamic libdrm)${NC}"
        if [[ $USE_DRM -eq 1 ]]; then
            echo -e "    ${YELLOW}➔ Active backend: DRM/KMS (Note: static fbdev recommended for shutdown)${NC}"
        else
            echo -e "    ${GREEN}➔ Active backend: Framebuffer (fbdev) [Universal]${NC}"
        fi
        echo -e "    ${GRAY}ℹ${CYAN} (toggle backend anytime via option T in main menu)${NC}"
    else
        echo -e "    ${YELLOW}☉ DRM/KMS: libdrm not detected, using Framebuffer fbdev${NC}"
    fi
...
# Check and optionally configure silent boot parameters in GRUB
check_silent_boot() {
    local interactive="${1:-1}"
    
    echo ""
    echo -e "${BLUE}   ════════════════════════════════════════════════════════════════${NC}"
    echo -e "${BOLD}${CYAN}                   SILENT BOOT (GRUB) DIAGNOSTIC                   ${NC}"
    echo -e "${BLUE}   ════════════════════════════════════════════════════════════════${NC}"
    echo -e "  ${GRAY}A silent boot suppresses kernel dmesg, systemd startup logs, and${NC}"
    echo -e "  ${GRAY}console cursor blinking so they do not disrupt the bootsplash.${NC}\n"
    ...
    if [[ -f "$grub_file" ]]; then
        grub_line=$(grep -E '^[[:space:]]*GRUB_CMDLINE_LINUX_DEFAULT=' "$grub_file" 2>/dev/null | head -1 | cut -d= -f2- | tr -d '"' | tr -d "'")
    fi
    
    local -a params
    params=(
        "quiet:quiet:quiet:Suppresses normal informational kernel messages:0"
        "splash:splash:splash:Signals that a graphical bootsplash is active:0"
        "loglevel:loglevel=:loglevel=0:Silences kernel dmesg console output:1"
        "systemd.show_status:systemd.show_status=:systemd.show_status=0:Hides systemd service startup logs:1"
        "rd.systemd.show_status:rd.systemd.show_status=:rd.systemd.show_status=false:Hides systemd status messages during initramfs:1"
        "rd.udev.log_level:rd.udev.log_level=:rd.udev.log_level=0:Silences udev device discovery logs in initramfs:1"
        "vt.cur_default:vt.cur_default=:vt.cur_default=1:Disables virtual terminal blinking cursor:1"
    )
    
    local missing_params=()
    local configured_count=0
    
    printf "  %-24s %-26b %-26b\n" "Parameter" "In GRUB" "Active Kernel"
    echo -e "  ${GRAY}─────────────────────────────────────────────────────────────────${NC}"
    
    for entry in "${params[@]}"; do
        local name pfx rec desc is_pfx
        IFS=":" read -r name pfx rec desc is_pfx <<< "$entry"
        local in_grub=0
        local in_cmdline=0
        ...
        [[ "$grub_line" =~ $reg ]] && in_grub=1
        [[ "$cmdline" =~ $reg ]] && in_cmdline=1
        
        local grub_str="${RED}[✗] Missing${NC}"
        if [[ $in_grub -eq 1 ]]; then
            grub_str="${GREEN}[✓] Configured${NC}"
            configured_count=$((configured_count + 1))
        else
            missing_params+=("$rec")
        fi
        
        local cmd_str="${YELLOW}[--] Inactive${NC}"
        [[ $in_cmdline -eq 1 ]] && cmd_str="${GREEN}[✓] Active${NC}"
        
        printf "  %-24s %-26b %-26b\n" "$name" "$grub_str" "$cmd_str"
    done
    
    echo -e "  ${GRAY}─────────────────────────────────────────────────────────────────${NC}"
    echo -e "  ${BOLD}Status:${NC} ${configured_count}/7 Silent Boot parameters configured in GRUB."
    echo ""
    echo -e "  ${CYAN}Current GRUB_CMDLINE_LINUX_DEFAULT:${NC}"
    echo -e "    ${GRAY}\"${grub_line}\"${NC}"
    echo ""
    
    if [[ ${#missing_params[@]} -eq 0 ]]; then
        echo -e "  ${GREEN}✓ All 7 recommended parameters are present in GRUB. Your boot is optimal!${NC}\n"
        return 0
    fi
    
    echo -e "  ${YELLOW}Missing parameter(s) to add:${NC}"
    ...
    if [[ "$interactive" -eq 1 ]]; then
        echo -en "${YELLOW}➤ Apply recommended settings to GRUB (/etc/default/grub)? [${CYAN}Y${YELLOW}/n]: ${NC}"
        read -r apply_choice
        if [[ "$apply_choice" =~ ^[Nn]$ ]]; then
            echo -e "  ${GRAY}No changes made to GRUB.${NC}"
            return 0
        fi
        
        if [[ $EUID -ne 0 ]]; then
            print_info "Root privileges required to edit GRUB and run update-grub."
            exec sudo "$0" --setup-silent-boot
        fi
        
        apply_silent_boot "${missing_params[@]}"
    fi
}

# Apply missing flags to /etc/default/grub
apply_silent_boot() {
    local to_add=("$@")
    local grub_file="/etc/default/grub"
    if [[ ! -f "$grub_file" ]]; then
    ...
    trap 'echo -e "  ${RED}[ROLLBACK] Error occurred, restoring $grub_file from backup...${NC}"; cp -a "$backup" "$grub_file" 2>/dev/null || true' ERR

    python3 - "${to_add[*]}" << 'PY_EOF'
import re, sys
...
if m:
    ...
    if to_add:
        ...
        with open(path, "w") as f:
            f.write(new_content)
        print("Updated GRUB_CMDLINE_LINUX_DEFAULT with: " + " ".join(to_add))
PY_EOF
    
    echo -e "  ${CYAN}Regenerating bootloader configuration...${NC}"
    ...
    trap - ERR
    echo -e "  ${GREEN}✓ Silent boot configuration applied successfully!${NC}"
    echo -e "  ${GRAY}The new settings will take effect on the next boot.${NC}\n"
}
...
show_bootsplash_status() {
    ...
    if [[ -d /etc/initramfs-tools/scripts/local-premount ]]; then
        local localpre=$(find /etc/initramfs-tools/scripts/local-premount -name "00_xbs_*" -type f 2>/dev/null)
        if [[ -n "$localpre" ]]; then
            echo -e "${NC}      Local-premount scripts:"
            for s in $localpre; do
                echo -e "${NC}        - $s"
            done
            has_files=true
        fi
...
uninstall_all_splash() {
    ...
    local xbs_files=""
    xbs_files=$(find /etc/initramfs-tools/hooks \( -name "xbs_*" -o -name "xbootsplash*" \) -type f 2>/dev/null)
    xbs_files="$xbs_files $(find /etc/initramfs-tools/scripts/init-top \( -name "xbs_*" -o -name "xbootsplash*" \) -type f 2>/dev/null)"
    xbs_files="$xbs_files $(find /etc/initramfs-tools/scripts/init-bottom \( -name "xbs_*" -o -name "xbootsplash*" \) -type f 2>/dev/null)"
    xbs_files="$xbs_files $(find /etc/initramfs-tools/scripts/local-premount \( -name "00_xbs_*" -o -name "00_xbootsplash*" \) -type f 2>/dev/null)"
    xbs_files="$xbs_files $(find /sbin \( -name "xbs_*" -o -name "xbootsplash*" \) -type f 2>/dev/null)"
    xbs_files="$xbs_files $(find /lib/systemd/system-shutdown \( -name "xbs_*" -o -name "xbootsplash*" \) -type f 2>/dev/null)"
    ...
    if [[ "$has_xbs" == "true" ]]; then
        ...
        find /etc/initramfs-tools/hooks \( -name "xbs_*" -o -name "xbootsplash*" \) -type f -exec rm -f {} \; 2>/dev/null
        find /etc/initramfs-tools/scripts/init-top \( -name "xbs_*" -o -name "xbootsplash*" \) -type f -exec rm -f {} \; 2>/dev/null
        find /etc/initramfs-tools/scripts/init-bottom \( -name "xbs_*" -o -name "xbootsplash*" \) -type f -exec rm -f {} \; 2>/dev/null
        find /etc/initramfs-tools/scripts/local-premount \( -name "00_xbs_*" -o -name "00_xbootsplash*" \) -type f -exec rm -f {} \; 2>/dev/null
        find /sbin \( -name "xbs_*" -o -name "xbootsplash*" \) -type f -exec rm -f {} \; 2>/dev/null
        find /lib/systemd/system-shutdown \( -name "xbs_*" -o -name "xbootsplash*" \) -type f -exec rm -f {} \; 2>/dev/null
        rm -f /lib/systemd/system-shutdown/bootsplash.shutdown
...
show_main_menu() {
    ...
    if [[ $LIBDRM_AVAILABLE -eq 1 ]]; then
        if [[ $USE_DRM -eq 1 ]]; then
           echo -e "${CYAN}T)${NC} ${BOLD}Toggle backend${NC} (DRM: boot-only) ${YELLOW_GRAD4}│"
        else
           echo -e "${CYAN}T)${NC} ${BOLD}Toggle backend${NC} (fbdev: universal)${YELLOW_GRAD4}│"
        fi
    fi
    echo -e "  ${YELLOW_GRAD4}│  ${CYAN}4)${NC} ${BOLD}Status${NC} splash               ${CYAN}S)${NC} ${BOLD}Silent boot${NC} (GRUB check)    ${YELLOW_GRAD4}│"
    echo -e "  ${YELLOW_GRAD4}│  ${BOLD}${YELLOW_GRAD5}Q)${NC} ${BOLD}Quit${NC}                                                        ${YELLOW_GRAD4}│"
    echo -e "  ${YELLOW_GRAD4}┕━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┙ "
    echo -en " ${YELLOW}  ➤ Select option [${CYAN}1${YELLOW}]: ${NC}"
     read -r main_choice
    ...
    case "$main_choice" in
        ...
        [Tt])
            if [[ $LIBDRM_AVAILABLE -eq 1 ]]; then
                ...
                if [[ $USE_DRM -eq 1 ]]; then
                    echo ""
                    echo -e "  ${YELLOW}⚠ Notice: DRM/KMS backend enabled (Specialized for Boot with tear-free VSync).${NC}"
                    echo -e "  ${YELLOW}  Dynamic DRM mode is not guaranteed during shutdown (shared libraries unmounted).${NC}"
                    echo -e "  ${YELLOW}  For shutdown splash or universal compatibility, static fbdev is recommended.${NC}"
                    sleep 1
                fi
...
if [ -n "$CHECK_SILENT_BOOT" ]; then
    check_silent_boot 0
    exit 0
fi

if [ -n "$SETUP_SILENT_BOOT" ]; then
    check_silent_boot 1
    exit 0
fi
