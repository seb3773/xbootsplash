...
static void decompress_palette_zx0(const uint8_t *compressed, size_t comp_size,
    ...
    if (dec_len > 0) {
        for (int i = 0; i < pixel_count; i++) {
            ...
            dst[i] = palette[val < palette_size ? val : 0];
        }
    }
    free(indices);
}
...
static void draw_resume_banner_drm(uint8_t *fb, int fb_w, int fb_h, int fb_pitch) {
    ...
    for (int by = 0; by < bh; by++) {
        ...
        for (int bx = 0; bx < bw; bx++) {
            ...
            uint32_t *p = (uint32_t *)(fb + py * fb_pitch + px * 4);
            uint32_t cur = *p;
            uint32_t r = (cur >> 16) & 0xFF;
            uint32_t g = (cur >> 8) & 0xFF;
            uint32_t b = cur & 0xFF;

            uint32_t out_r = (r * inv_a + tc * a) / 255;
            uint32_t out_g = (g * inv_a + tc * a) / 255;
            uint32_t out_b = (b * inv_a + tc * a) / 255;

            *p = (out_r << 16) | (out_g << 8) | out_b;
        }
    }
}
#endif
...
static void sleep_ms(unsigned int ms) {
    ...
    struct timespec rem;
    while (nanosleep(&req, &rem) != 0 && !terminate_requested && !resume_requested) {
        req = rem;
    }
}
...
int main(int argc, char **argv) {
    ...
    int tty_gfd = open("/dev/tty0", O_RDWR, 0);
    if (tty_gfd < 0) tty_gfd = open("/dev/tty", O_RDWR, 0);
    if (tty_gfd < 0) tty_gfd = open("/dev/console", O_RDWR, 0);
    if (tty_gfd >= 0) {
        ioctl(tty_gfd, KDSETMODE, KD_GRAPHICS);
        close(tty_gfd);
    }
...
#ifdef HAS_RESUME_BANNER
        if (resume_requested) {
            draw_resume_banner_drm(drm_ctx.map, drm_ctx.width, drm_ctx.height, drm_ctx.pitch);
            if (drmModeDirtyFB(drm_ctx.fd, drm_ctx.fb_id, NULL, 0) != 0) {
    ...
    while (!force_exit) {
        ...
        if (drmModeDirtyFB(drm_ctx.fd, drm_ctx.fb_id, NULL, 0) != 0) {
            /* Fallback to SetCrtc if DirtyFB is not supported */
            drmModeSetCrtc(drm_ctx.fd, drm_ctx.crtc_id, drm_ctx.fb_id, 0, 0,
                           &drm_ctx.conn_id, 1, &drm_ctx.mode);
        }
...
#ifdef HAS_RESUME_BANNER
                if (resume_requested && !banner_drawn) {
                    draw_resume_banner_drm(drm_ctx.map, drm_ctx.width, drm_ctx.height, drm_ctx.pitch);
                    if (drmModeDirtyFB(drm_ctx.fd, drm_ctx.fb_id, NULL, 0) != 0) {
                        drmModeSetCrtc(drm_ctx.fd, drm_ctx.crtc_id, drm_ctx.fb_id, 0, 0,
                                       &drm_ctx.conn_id, 1, &drm_ctx.mode);
