...
#include <png.h>
#include <fcntl.h>
#include "zx0/zx0_compress.c"
#include "upkr/upkr_compress.c"
...
static int loop_mode = 1;      /* 0=no loop, 1=full loop, 2=partial loop, 3=ping-pong loop */
static int loop_start = 0;     /* Start frame for partial loop */
static int min_boot_loops = 0; /* Minimum complete loops at boot before allowing exit (0=disabled) */
...
#define MODE_ANIM_IMAGE_CENTER 1  /* Animation on background image on uniform color */
#define MODE_ANIM_IMAGE_FULL   2  /* Animation on fullscreen background image */
#define MODE_STATIC_CENTER     3  /* Static image on solid background */
...
#define COMPRESS_PALETTE_ZX0  6  /* Static image / background: 8-bit palette + ZX0 */
#define COMPRESS_PALETTE_UPKR 7  /* Static image / background: 8-bit palette + UPKR */
static int compress_method = COMPRESS_RLE_XOR;
static int super_compression = 0; /* 0 = normal, 1 = ZX0, 2 = UPKR */
static int use_zx0 = 0;          /* 0 = normal (fast, LZSS / direct frames), 1 = ZX0 super-compression */
...
/* Natural order string comparison (case-insensitive, handles numbers anywhere) */
static int natural_sort_cmp(const char *s1, const char *s2) {
    if (!s1 && !s2) return 0;
    if (!s1) return -1;
    if (!s2) return 1;
...
/* Compare frames by filename using natural sorting */
static int compare_frames_by_natural_path(const void *a, const void *b) {
    const char *path_a = ((const frame_entry_t*)a)->path;
    const char *path_b = ((const frame_entry_t*)b)->path;
    ...
    return natural_sort_cmp(name_a, name_b);
}

static int compare_frames(const void *a, const void *b) {
    return ((const frame_entry_t*)a)->index - ((const frame_entry_t*)b)->index;
}

#define MAX_NUMS_PER_NAME 16

typedef struct {
    int num_count;
    long nums[MAX_NUMS_PER_NAME];
} name_numbers_t;

static void extract_numbers_from_filename(const char *filename, name_numbers_t *out) {
    out->num_count = 0;
    const char *p = filename;
    while (*p && out->num_count < MAX_NUMS_PER_NAME) {
        if (isdigit((unsigned char)*p)) {
            char *end = NULL;
            out->nums[out->num_count++] = strtol(p, &end, 10);
            p = end;
        } else {
            p++;
        }
...
/* Check if an image file is an intruder (background, preview, thumb, or non-numbered file when others are numbered) */
static int is_intruder_image(const char *filename, int has_digit_majority) {
    ...
    if (has_digit_majority) {
        int has_digit = 0;
        for (size_t i = 0; i < len; i++) {
            if (isdigit((unsigned char)lower[i])) {
                has_digit = 1;
                break;
            }
        }
...
/* Collect, filter, and sort animation frames from input directory */
static int collect_and_sort_frames(const char *input_path, frame_entry_t **out_frames,
                                   int *out_nframes, int invert_frames) {
    ...
    /* Count total images and check if numbers are present */
    int total_images = 0;
    int images_with_digits = 0;
    struct dirent *ent;
    while ((ent = readdir(dir)) != NULL) {
    ...
    if (total_images == 0) {
        fprintf(stderr, "Error: No image frames found in %s\n", input_path);
        closedir(dir);
        return 1;
    }
    ...
    frame_entry_t *frames = malloc(sizeof(frame_entry_t) * total_images);
    int nframes = 0;
    char *excluded[MAX_FRAMES];
    int n_excluded = 0;
    ...
    while ((ent = readdir(dir)) != NULL) {
        if (strstr(ent->d_name, ".png") || strstr(ent->d_name, ".PNG") ||
            ...
            int intruder = is_intruder_image(ent->d_name, images_with_digits > 0);
            if (intruder) {
                excluded[n_excluded++] = strdup(ent->d_name);
            } else {
    ...
    /* If all images were excluded (e.g. naming without digits), restore them */
    if (nframes == 0 && n_excluded > 0) {
        for (int i = 0; i < n_excluded; i++) {
            frames[nframes].path = malloc(512);
            frames[nframes].tmp_path = malloc(512);
            snprintf(frames[nframes].path, 512, "%s/%s", input_path, excluded[i]);
            snprintf(frames[nframes].tmp_path, 512, "%s/%s", tmpdir, excluded[i]);
            frames[nframes].index = -1;
            nframes++;
            free(excluded[i]);
        }
    ...
    if (n_excluded > 0) {
        fprintf(stderr, "Note: Excluded %d non-animation file(s) from sequence:\n", n_excluded);
        for (int i = 0; i < n_excluded; i++) {
            fprintf(stderr, "  - %s\n", excluded[i]);
            free(excluded[i]);
        }
    ...
    fprintf(stderr, "Found %d valid animation frames\n", nframes);

    /* 1. Sort by filename naturally */
    qsort(frames, nframes, sizeof(frame_entry_t), compare_frames_by_natural_path);

    /* 2. Check if a sequence column uniquely identifies frames */
    if (nframes > 1) {
        name_numbers_t *nums = malloc(sizeof(name_numbers_t) * nframes);
        int min_nums = 99;
        for (int i = 0; i < nframes; i++) {
            const char *fn = strrchr(frames[i].path, '/');
            fn = fn ? fn + 1 : frames[i].path;
            extract_numbers_from_filename(fn, &nums[i]);
            if (nums[i].num_count < min_nums) min_nums = nums[i].num_count;
        }
        ...
        if (min_nums > 0) {
            for (int col = 0; col < min_nums; col++) {
                int unique = 0;
                long min_v = nums[0].nums[col];
                long max_v = nums[0].nums[col];
                for (int i = 0; i < nframes; i++) {
                    long v = nums[i].nums[col];
                    if (v < min_v) min_v = v;
                    if (v > max_v) max_v = v;
                    int is_u = 1;
                    for (int j = 0; j < i; j++) {
                        if (nums[j].nums[col] == v) { is_u = 0; break; }
                    }
                    if (is_u) unique++;
                }
                ...
                if (unique == nframes) {
                    int score = 1000;
                    if (max_v - min_v + 1 == nframes) score += 500;
                    score += col;
                    if (score > best_score) {
                        best_score = score;
                        best_col = col;
        ...
        if (best_col >= 0) {
            for (int i = 0; i < nframes; i++) {
                frames[i].index = nums[i].nums[best_col];
            }
    ...
    /* Assign sequential 0..nframes-1 indices */
    for (int i = 0; i < nframes; i++) {
        frames[i].index = i;
    }
...
/*
 * In Ping-Pong loop mode (loop_mode == 3), expand animation frames:
 * 0, 1, ..., N-1, N-2, ..., 1.
 * First and last frames are not duplicated to prevent pauses at the endpoints.
 */
static void apply_pingpong_sequence(image_t **imgs_ptr, int *nframes_ptr) {
    int n = *nframes_ptr;
    if (loop_mode != 3 || n <= 2) return;

    int pp_count = 2 * n - 2;
    image_t *pp_imgs = malloc(sizeof(image_t) * pp_count);
    if (!pp_imgs) {
        fprintf(stderr, "Error: Out of memory expanding ping-pong frames\n");
        exit(1);
    }

    /* Forward sequence: 0 .. n-1 */
    for (int i = 0; i < n; i++) {
        pp_imgs[i] = (*imgs_ptr)[i];
    }

    /* Reverse sequence: n-2 down to 1 */
    int dst_idx = n;
    for (int i = n - 2; i >= 1; i--) {
        pp_imgs[dst_idx].w = (*imgs_ptr)[i].w;
        pp_imgs[dst_idx].h = (*imgs_ptr)[i].h;
        size_t sz = sizeof(uint32_t) * pp_imgs[dst_idx].w * pp_imgs[dst_idx].h;
        pp_imgs[dst_idx].pixels = malloc(sz);
        if (!pp_imgs[dst_idx].pixels) {
            fprintf(stderr, "Error: Out of memory copying ping-pong frame pixels\n");
            exit(1);
        }
        memcpy(pp_imgs[dst_idx].pixels, (*imgs_ptr)[i].pixels, sz);
        dst_idx++;
    }

    free(*imgs_ptr);
    *imgs_ptr = pp_imgs;
    *nframes_ptr = pp_count;
    fprintf(stderr, "Ping-Pong mode enabled: animation expanded to %d frames (0..%d..1)\n",
            pp_count, n - 1);
}
...
/* Load raw alpha channel from PNG without flattening */
static uint8_t *load_alpha_mask_png(const char *path, int *out_w, int *out_h) {
    FILE *fp = fopen(path, "rb");
    if (!fp) return NULL;
    ...
    uint8_t *alphas = (uint8_t*)malloc(w * h);
    if (alphas) {
        for (int y = 0; y < h; y++) {
            for (int x = 0; x < w; x++) {
                alphas[y * w + x] = row_pointers[y][x * 4 + 3];
            }
            free(row_pointers[y]);
        }
    } else {
...
static const char *find_resume_icon(int is_hd) {
    static char found_path[512];
    const char *fn = is_hd ? "resume_hd.png" : "resume.png";
    const char *candidates[] = {
        "icons",
        "GUI_src/icons",
        "GUI/icons",
        "/usr/share/xbootsplash/icons",
        NULL
    };
    for (int i = 0; candidates[i]; i++) {
        snprintf(found_path, sizeof(found_path), "%s/%s", candidates[i], fn);
...
static void output_resume_banner(int screen_w, int screen_h, const image_t *bg_image, uint32_t bg_hex) {
    int is_hd = (screen_w > 1920);
    const char *icon_path = find_resume_icon(is_hd);
    if (!icon_path) {
        fprintf(stderr, "Notice: Resume banner icon not found (checked GUI/icons/%s)\n", is_hd ? "resume_hd.png" : "resume.png");
        return;
    }
    
    int bw = 0, bh = 0;
    uint8_t *alphas = load_alpha_mask_png(icon_path, &bw, &bh);
    if (!alphas || bw <= 0 || bh <= 0) {
        fprintf(stderr, "Warning: Failed to load alpha mask from %s\n", icon_path);
        if (alphas) free(alphas);
        return;
    }
    
    int banner_x = (screen_w - bw) / 2;
    int banner_y = is_hd ? 45 : 30;
    if (banner_x < 0) banner_x = 0;
    
    /* Calculate average luminance in banner region */
    long total_lum = 0;
    long sampled_count = 0;
    
    if (bg_image && bg_image->pixels && bg_image->w > 0 && bg_image->h > 0) {
        for (int by = 0; by < bh; by++) {
            int py = banner_y + by;
            if (py >= bg_image->h) break;
            for (int bx = 0; bx < bw; bx++) {
                int px = banner_x + bx;
                if (px >= bg_image->w) break;
                
                uint16_t pix = bg_image->pixels[py * bg_image->w + px];
                int r = ((pix >> 11) & 0x1F) * 255 / 31;
                int g = ((pix >> 5) & 0x3F) * 255 / 63;
                int b = (pix & 0x1F) * 255 / 31;
                total_lum += (299 * r + 587 * g + 114 * b) / 1000;
                sampled_count++;
            }
        }
    } else {
    ...
    int avg_lum = (sampled_count > 0) ? (int)(total_lum / sampled_count) : 0;
    int dark_text = (avg_lum >= 128) ? 1 : 0;
    
    fprintf(stderr, "Resume banner: %dx%d (icon: %s), Top luminance: %d/255 -> %s text\n",
            bw, bh, icon_path, avg_lum, dark_text ? "DARK" : "WHITE");
            
    printf("\n/* Hibernation Resume Banner ('Resume from hibernation...') */\n");
    printf("#define HAS_RESUME_BANNER 1\n");
    printf("#define RESUME_BANNER_W %d\n", bw);
    printf("#define RESUME_BANNER_H %d\n", bh);
    printf("#define RESUME_BANNER_Y %d\n", banner_y);
    printf("#define RESUME_BANNER_DARK_TEXT %d\n", dark_text);
    
    size_t packed_size = (bw * bh + 1) / 2;
    printf("static const uint8_t resume_banner_alpha_packed[%zu] = {\n    ", packed_size);
    
    for (size_t i = 0; i < packed_size; i++) {
        int idx1 = i * 2;
        int idx2 = idx1 + 1;
        uint8_t a1 = (idx1 < bw * bh) ? (alphas[idx1] >> 4) : 0;
        uint8_t a2 = (idx2 < bw * bh) ? (alphas[idx2] >> 4) : 0;
        uint8_t packed = (a1 << 4) | (a2 & 0x0F);
        
        printf("0x%02X%s", packed, (i + 1 < packed_size) ? "," : "");
        if ((i + 1) % 16 == 0 && i + 1 < packed_size) printf("\n    ");
        else if (i + 1 < packed_size) printf(" ");
    }
    printf("\n};\n\n");
...
/* Output palette + ZX0 compressed static image */
static void output_palette_zx0(const uint16_t *palette, int num_colors,
                               const uint8_t *compressed, size_t comp_size,
                               int w, int h) {
    (void)w; (void)h;
    printf("/* Palette + ZX0 compressed static image */\n");
    printf("#define PALETTE_SIZE %d\n", num_colors);
    printf("#define IMG_COMPRESSED_SIZE %zu\n\n", comp_size);
...
/* Output palette + UPKR compressed static image */
static void output_palette_upkr(const uint16_t *palette, int num_colors,
                                const uint8_t *compressed, size_t comp_size,
                                int w, int h) {
    (void)w; (void)h;
    printf("/* Palette + UPKR compressed static image */\n");
    printf("#define PALETTE_SIZE %d\n", num_colors);
    printf("#define IMG_COMPRESSED_SIZE %zu\n\n", comp_size);
...
/* Output animation frames either discrete (normal mode) or ZX0/UPKR super-packed */
static void output_animation_stream(uint8_t **compressed, size_t *comp_sizes, int nframes, int super_mode) {
    if (!super_mode) {
        ...
        for (int f = 0; f < nframes; f++) total_uncomp += comp_sizes[f];

        uint8_t *uncomp_buf = (uint8_t*)malloc(total_uncomp);
        if (!uncomp_buf) {
            fprintf(stderr, "Error: out of memory preparing animation stream\n");
            exit(1);
        }
        printf("#define USE_ZX0_ANIM 1\n");
        printf("#define TOTAL_UNCOMPRESSED_FRAMES_SIZE %zu\n\n", total_uncomp);
        printf("static const uint32_t frame_offsets[NFRAMES] = {\n");
        size_t offset = 0;
        for (int f = 0; f < nframes; f++) {
            printf("    %zu,\n", offset);
            memcpy(uncomp_buf + offset, compressed[f], comp_sizes[f]);
            offset += comp_sizes[f];
        }
        printf("};\n\n");
        printf("static const uint32_t frame_sizes[NFRAMES] = {\n");
        ...
        unsigned char *zx0_out = NULL;
        size_t zx0_sz = 0;
        fprintf(stderr, "Super-packing animation deltas with ZX0 (%zu bytes uncompressed)...\n", total_uncomp);
        if (zx0_compress_custom(uncomp_buf, total_uncomp, &zx0_out, &zx0_sz, 4096) != 0) {
            fprintf(stderr, "Error: ZX0 animation compression failed!\n");
            free(uncomp_buf);
            exit(1);
        }
        free(uncomp_buf);
        fprintf(stderr, "ZX0 packed animation: %zu bytes (%.1f%% of delta stream, %zu bytes saved)\n",
                zx0_sz, 100.0 * zx0_sz / (total_uncomp ? total_uncomp : 1), total_uncomp - zx0_sz);

        printf("static const uint8_t anim_frames_zx0[%zu] = {\n", zx0_sz);
        for (size_t i = 0; i < zx0_sz; i++) {
            if (i % 16 == 0) printf("    ");
            printf("0x%02X", zx0_out[i]);
            if (i < zx0_sz - 1) printf(",");
            if ((i + 1) % 16 == 0) printf("\n");
        }
        printf("\n};\n\n");
        free(zx0_out);
        ...
        size_t upkr_cap = total_uncomp * 2 + 1024;
        uint8_t *upkr_out = (uint8_t*)malloc(upkr_cap);
        if (!upkr_out) {
            fprintf(stderr, "Error: out of memory for UPKR output buffer\n");
            free(uncomp_buf);
            exit(1);
        }
        fprintf(stderr, "Super-packing animation deltas with UPKR (%zu bytes uncompressed)...\n", total_uncomp);
        size_t upkr_sz = upkr_compress(uncomp_buf, total_uncomp, upkr_out, upkr_cap, 6);
        if (upkr_sz == 0) {
            fprintf(stderr, "Error: UPKR animation compression failed!\n");
            free(uncomp_buf);
            free(upkr_out);
            exit(1);
        }
        free(uncomp_buf);
        fprintf(stderr, "UPKR packed animation: %zu bytes (%.1f%% of delta stream, %zu bytes saved)\n",
                upkr_sz, 100.0 * upkr_sz / (total_uncomp ? total_uncomp : 1), total_uncomp - upkr_sz);

        printf("static const uint8_t anim_frames_upkr[%zu] = {\n", upkr_sz);
        for (size_t i = 0; i < upkr_sz; i++) {
            if (i % 16 == 0) printf("    ");
            printf("0x%02X", upkr_out[i]);
            if (i < upkr_sz - 1) printf(",");
            if ((i + 1) % 16 == 0) printf("\n");
        }
        printf("\n};\n\n");
        free(upkr_out);
...
static void print_help(const char *prog) {
    ...
    fprintf(stderr, "  1 = Animation on background image on uniform color\n");
    fprintf(stderr, "  2 = Animation on background image (fullscreen)\n");
    fprintf(stderr, "  3 = Static image on solid background\n");
    ...
    fprintf(stderr, "  -l <mode>      Loop mode: 0=no loop, 1=full loop (default), 2=partial loop, 3=ping-pong loop\n");
    fprintf(stderr, "  -L <frame>     Loop start frame for partial loop mode (default: 0)\n");
    fprintf(stderr, "  -M <loops>     Minimum complete loops at boot before exit (default: 0 = exit immediately)\n");
    ...
    fprintf(stderr, "  -U, --upkr     Enable UPKR super-compression (fast build, maximum compression)\n");
    fprintf(stderr, "  -Z, --zx0      Enable ZX0 super-compression (slower build, smaller binary)\n");
...
int main(int argc, char *argv[]) {
    ...
    while (arg_idx < argc) {
        if (strcmp(argv[arg_idx], "-m") == 0 && arg_idx + 1 < argc) {
            ...
            arg_idx++;
        } else if ((strcmp(argv[arg_idx], "-y") == 0 || strcmp(argv[arg_idx], "-o") == 0) && arg_idx + 1 < argc) {
            ...
            if (strcmp(method, "rle_xor") == 0 || strcmp(method, "0") == 0) compress_method = COMPRESS_RLE_XOR;
            else if (strcmp(method, "rle_direct") == 0 || strcmp(method, "1") == 0) compress_method = COMPRESS_RLE_DIRECT;
            else if (strcmp(method, "sparse") == 0 || strcmp(method, "2") == 0) compress_method = COMPRESS_SPARSE;
            else if (strcmp(method, "raw") == 0 || strcmp(method, "3") == 0) compress_method = COMPRESS_RAW;
            else if (strcmp(method, "auto") == 0 || strcmp(method, "-1") == 0) compress_method = COMPRESS_AUTO;
            else fprintf(stderr, "Warning: Unknown compression method '%s', using default\n", method);
            arg_idx++;
        } else if (strcmp(argv[arg_idx], "-U") == 0 || strcmp(argv[arg_idx], "--upkr") == 0) {
            super_compression = 2;
            arg_idx++;
        } else if (strcmp(argv[arg_idx], "-Z") == 0 || strcmp(argv[arg_idx], "--zx0") == 0) {
            super_compression = 1;
            use_zx0 = 1;
            arg_idx++;
        } else if ((strcmp(argv[arg_idx], "-M") == 0 || strcmp(argv[arg_idx], "--min-loops") == 0) && arg_idx + 1 < argc) {
            min_boot_loops = atoi(argv[++arg_idx]);
            arg_idx++;
    ...
    const char *mode_names[] = {
        ...
        "Animation on background image on uniform color",
        "Animation on background image (fullscreen)",
        "Static image on solid background",
    ...
    if (display_mode == MODE_ANIM_SOLID || display_mode == MODE_ANIM_IMAGE_CENTER || display_mode == MODE_ANIM_IMAGE_FULL) {
        ...
        printf("#define LOOP_MODE %d  /* 0=no loop, 1=full loop, 2=partial loop, 3=ping-pong */\n", loop_mode);
        if (loop_mode == 2) {
            printf("#define LOOP_START %d  /* Loop restart frame for partial loop */\n", loop_start);
        }
        printf("#define MIN_BOOT_LOOPS %d  /* Minimum complete loops at boot before exit (0=instant) */\n", min_boot_loops);
    }
    ...
    if (display_mode == MODE_STATIC_CENTER || display_mode == MODE_STATIC_FULLSCREEN) {
        ...
        if (super_compression == 2) {
            uint8_t *upkr_compressed = (uint8_t*)malloc(pixel_count * 2 + 1024);
            fprintf(stderr, "Compressing static image with UPKR...\n");
            size_t upkr_comp_size = upkr_compress(indices, pixel_count, upkr_compressed, pixel_count * 2 + 1024, 6);
            if (upkr_comp_size == 0) {
                fprintf(stderr, "Error: UPKR compression failed\n");
                free(palette);
                free(indices);
                free(compressed);
                free(upkr_compressed);
                free(img.pixels);
                return 1;
            }
            fprintf(stderr, "UPKR compressed: %zu bytes (%.1f%% of raw)\n", 
                    upkr_comp_size, 100.0 * upkr_comp_size / (pixel_count * 2));
            
            /* Output compressed data */
            printf("#define COMPRESS_METHOD %d  /* PALETTE_UPKR */\n", COMPRESS_PALETTE_UPKR);
            output_palette_upkr(palette, num_colors, upkr_compressed, upkr_comp_size, img.w, img.h);
            free(upkr_compressed);
        } else if (super_compression == 1 || use_zx0) {
            unsigned char *zx0_compressed = NULL;
            size_t zx0_comp_size = 0;
            fprintf(stderr, "Compressing static image with ZX0...\n");
            if (zx0_compress_custom(indices, pixel_count, &zx0_compressed, &zx0_comp_size, 4096) != 0) {
                fprintf(stderr, "Error: ZX0 compression failed\n");
                free(palette);
                free(indices);
                free(compressed);
                free(img.pixels);
            ...
            fprintf(stderr, "ZX0 compressed: %zu bytes (%.1f%% of raw)\n", 
                    zx0_comp_size, 100.0 * zx0_comp_size / (pixel_count * 2));
            
            /* Output compressed data */
            printf("#define COMPRESS_METHOD %d  /* PALETTE_ZX0 */\n", COMPRESS_PALETTE_ZX0);
            output_palette_zx0(palette, num_colors, zx0_compressed, zx0_comp_size, img.w, img.h);
            free(zx0_compressed);
        } else {
        ...
        output_resume_banner(img.w, img.h, &img, bg_color);
        ...
        int nframes = 0;
        if (collect_and_sort_frames(input_path, &frames, &nframes, invert_frames) != 0) {
            return 1;
        }
        ...
        /* Free frames path entries as paths are no longer needed */
        for (int i = 0; i < nframes; i++) {
            free(frames[i].path);
        ...
        apply_pingpong_sequence(&frame_imgs, &nframes);
        ...
        if (compress_method == COMPRESS_AUTO) {
            ...
            output_animation_stream(compressed, comp_sizes, nframes, super_compression);
            output_resume_banner(target_w > 0 ? target_w : 1920, target_h > 0 ? target_h : 1080, NULL, bg_color);
        ...
        if (!bg_image_path || bg_image_path[0] == '\0') {
        ...
        if (super_compression == 2) {
            uint8_t *bg_upkr_comp = malloc(bg_pixel_count * 2 + 1024);
            fprintf(stderr, "Compressing background image with UPKR...\n");
            size_t bg_upkr_comp_size = upkr_compress(bg_indices, bg_pixel_count, bg_upkr_comp, bg_pixel_count * 2 + 1024, 6);
            if (bg_upkr_comp_size == 0) {
                fprintf(stderr, "Error: Background UPKR compression failed\n");
                free(bg_palette);
                free(bg_indices);
                free(bg_compressed);
                free(bg_upkr_comp);
            ...
            fprintf(stderr, "Background UPKR: %zu bytes (%.1f%% of raw)\n", 
                    bg_upkr_comp_size, 100.0 * bg_upkr_comp_size / (bg_pixel_count * 2));
            
            printf("#define BG_COMPRESS_METHOD %d  /* PALETTE_UPKR */\n", COMPRESS_PALETTE_UPKR);
            printf("#define BG_PALETTE_SIZE %d\n", bg_num_colors);
            printf("#define BG_COMPRESSED_SIZE %zu\n\n", bg_upkr_comp_size);
            
            output_bg_palette_lzss(bg_palette, bg_num_colors, bg_upkr_comp, bg_upkr_comp_size);
            free(bg_upkr_comp);
        } else if (super_compression == 1 || use_zx0) {
            unsigned char *bg_zx0_comp = NULL;
            size_t bg_zx0_comp_size = 0;
            fprintf(stderr, "Compressing background image with ZX0...\n");
            if (zx0_compress_custom(bg_indices, bg_pixel_count, &bg_zx0_comp, &bg_zx0_comp_size, 4096) != 0) {
                fprintf(stderr, "Error: Background ZX0 compression failed\n");
                free(bg_palette);
                free(bg_indices);
                free(bg_compressed);
                free(bg.pixels);
            ...
            fprintf(stderr, "Background ZX0: %zu bytes (%.1f%% of raw)\n", 
                    bg_zx0_comp_size, 100.0 * bg_zx0_comp_size / (bg_pixel_count * 2));
            
            printf("#define BG_COMPRESS_METHOD %d  /* PALETTE_ZX0 */\n", COMPRESS_PALETTE_ZX0);
            printf("#define BG_PALETTE_SIZE %d\n", bg_num_colors);
            printf("#define BG_COMPRESSED_SIZE %zu\n\n", bg_zx0_comp_size);
            
            output_bg_palette_lzss(bg_palette, bg_num_colors, bg_zx0_comp, bg_zx0_comp_size);
            free(bg_zx0_comp);
        } else {
            ...
            printf("#define BG_COMPRESS_METHOD %d  /* PALETTE_LZSS */\n", COMPRESS_PALETTE_LZSS);
            printf("#define BG_PALETTE_SIZE %d\n", bg_num_colors);
            printf("#define BG_COMPRESSED_SIZE %zu\n\n", bg_comp_size);
        ...
        free(bg_indices);
        free(bg_compressed);
        /* Keep bg.pixels until output_resume_banner is finished */
        ...
        if (compress_method == COMPRESS_AUTO) {
            ...
            output_animation_stream(compressed, comp_sizes, nframes, super_compression);
            output_resume_banner(bg.w, bg.h, &bg, bg_color);
            if (bg.pixels) { free(bg.pixels); bg.pixels = NULL; }
